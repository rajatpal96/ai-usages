import { UsageEventModel, Session, McpToolCall, } from './index.js';
import { parseTimeRange } from '../../common/src/index.js';
export class AnalyticsService {
    /**
     * GET /v1/analytics/overview
     */
    async getOverview(organizationId, rangeStr = '30d') {
     ...
     */
    async getUsageTrend(organizationId, rangeStr = '30d') {
        const { start, end } = parseTimeRange(rangeStr);
     ...
     */
    async getUsageByAgent(organizationId, rangeStr = '30d') {
        const { start, end } = parseTimeRange(rangeStr);
        return UsageEventModel.aggregate([
            {
                $match: {
     ...
     */
    async getUsageByModel(organizationId, rangeStr = '30d') {
        const { start, end } = parseTimeRange(rangeStr);
        return UsageEventModel.aggregate([
            {
                $match: {
     ...
     */
    async getUsageByProject(organizationId, rangeStr = '30d') {
        const { start, end } = parseTimeRange(rangeStr);
        return UsageEventModel.aggregate([
            {
                $match: {
     ...
     */
    async getUsageByUser(organizationId, rangeStr = '30d') {
        const { start, end } = parseTimeRange(rangeStr);
        return UsageEventModel.aggregate([
            {
                $match: {
     ...
     * GET /v1/sessions
     */
    async listSessions(organizationId, options) {
        const query = { organizationId };
        if (options.agent)
            query.agentName = options.agent;
        if (options.project)
     ...
     */
    async getMcpAnalytics(organizationId, rangeStr = '30d') {
        const { start, end } = parseTimeRange(rangeStr);
