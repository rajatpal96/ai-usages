...
import { connectDatabase, Organization, User, Project, Session, UsageEventModel, McpToolCall, Pricing, AlertRuleModel, BudgetModel, ApiKey, } from './index.js';
...
export async function seedDatabase(organizationId = 'org_default') {
    await connectDatabase();
    logger.info({ organizationId }, 'Starting database seeding for AgentMeter fleet...');
    // 1. Organization
    await Organization.findOneAndUpdate({ orgId: organizationId }, { orgId: organizationId, name: 'Acme Engineering', plan: 'enterprise' }, { upsert: true });
    // 2. Users
    const users = [
    ...
    for (const tier of DEFAULT_PRICING_CATALOG) {
        await Pricing.findOneAndUpdate({ provider: tier.provider, model: tier.model }, {
    ...
    await BudgetModel.findOneAndUpdate({ organizationId, month: currentMonth }, {
        organizationId,
    ...
    for (const r of alertRules) {
        await AlertRuleModel.findOneAndUpdate({ organizationId, name: r.name }, r, { upsert: true });
    }
