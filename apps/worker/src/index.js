...
import { connectDatabase, UsageHourly, UsageDaily, Session, BudgetModel, AlertRuleModel, AlertHistory, } from '../../../packages/database/src/index.js';
...
export class BackgroundWorker {
    ...
    async start() {
        ...
        await kafkaClient.subscribeConsumer(KAFKA_TOPICS.USAGE_EVENTS, 'agentmeter-worker-group', this.processUsageEvent.bind(this));
        await kafkaClient.subscribeConsumer(KAFKA_TOPICS.MCP_EVENTS, 'agentmeter-worker-group', this.processMcpEvent.bind(this));
     ...
     */
    async processMcpEvent(mcpEvent) {
        try {
            if (mcpEvent.latencyMs > 3000) {
                log.warn({ tool: mcpEvent.toolName, latencyMs: mcpEvent.latencyMs }, 'High MCP tool latency detected');
            }
        }
        catch (err) {
     ...
     */
    async evaluateAlertsAndBudgets(event) {
        try {
            ...
            if (budget) {
                ...
                if (usagePercent >= budget.alertThresholdPercent) {
                    log.warn({ orgId, currentSpend: budget.currentSpendUsd, limit: budget.monthlyLimitUsd, percent: usagePercent }, '⚠️ Organization monthly budget threshold reached!');
    ...
    async triggerAlert(rule, triggeredValue, message) {
