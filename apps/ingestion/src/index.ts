...
async function authMiddleware(req: Request, res: Response, next: NextFunction) {
  try {
    ...
    if (userProfile) {
      ...
      (req as any).userId = userProfile.userId;
...
app.post('/v1/events', authMiddleware, async (req: Request, res: Response) => {
  ...
  try {
    ...
    if (!parsedEvent.cost || parsedEvent.cost.total === 0) {
      parsedEvent.cost = defaultCostEngine.calculateCost(
        parsedEvent.provider.name,
        parsedEvent.model.name,
        parsedEvent.usage
      );
    }
    ...
    for (const raw of rawEvents) {
      ...
      if (!seenEventIds.has(parsed.eventId)) {
        ...
        if (!parsed.cost || parsed.cost.total === 0) {
          parsed.cost = defaultCostEngine.calculateCost(
...
// -------------------------------------------------------------
app.post('/v1/mcp/events', authMiddleware, async (req: Request, res: Response) => {
...
// -------------------------------------------------------------
app.post(['/v1/chat/completions', '/api/v1/chat/completions'], authMiddleware, async (req: Request, res: Response) => {
  const orgId = (req as any).organizationId || config.DEFAULT_ORG_ID;
...
export async function startIngestionServer(port: number = config.INGESTION_PORT): Promise<any> {
  ...
  return new Promise((resolve, reject) => {
    const server = app.listen(port, () => {
      ...
      log.info({ port: actualPort }, `⚡ AgentMeter Ingestion Service running on http://localhost:${actualPort}`);
    ...
    server.on('error', (err: any) => {
      if (err.code === 'EADDRINUSE') {
        ...
        log.warn({ port, fallbackPort }, `Ingestion server port ${port} is already in use by another process. Binding test instance to fallback port ${fallbackPort}...`);
