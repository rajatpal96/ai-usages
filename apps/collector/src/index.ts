...
import { UsageEvent, UsageEventSchema } from '../../../packages/event-schema/src/index.js';
...
export class LocalCollector {
   ...
   */
  public enqueue(rawEvent: any): boolean {
    try {
      const normalized: UsageEvent = adapterRegistry.normalize(rawEvent, {
  ...
  public async flush(): Promise<number> {
    ...
    try {
      ...
      const rows: any[] = select.all(this.config.batchSize);
   ...
   */
  private async uploadBatch(events: UsageEvent[]): Promise<boolean> {
    try {
  ...
  public async doctor(): Promise<DoctorReport> {
    const queueDepthRow: any = this.db.prepare(`SELECT COUNT(*) as count FROM events WHERE status = 'pending'`).get();
    const quarantinedRow: any = this.db.prepare(`SELECT COUNT(*) as count FROM events WHERE status = 'quarantined'`).get();
